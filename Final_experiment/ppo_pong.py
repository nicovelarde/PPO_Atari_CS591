...
class CNNFeatureExtractor(nn.Module):
    ...
    Input: Single grayscale frame (84x84) → shape (batch, 1, 84, 84)
    Output: Feature vector of size 512 → shape (batch, 512)

    Uses 84x84 grayscale images following the DQN/Atari preprocessing standard.
    Architecture based on: https://towardsdatascience.com/getting-an-ai-to-play-atari-pong-with-deep-reinforcement-learning-47b0c56e78ae/
    """

    def __init__(self, input_channels: int = 1, feature_dim: int = 512):
        super(CNNFeatureExtractor, self).__init__()

        # First convolutional layer: 1 channel (grayscale) → 32 filters
        # kernel_size=8, stride=4 → reduces spatial dimensions significantly
        ...
        # Third convolutional layer: 64 → 64 filters
        self.conv3 = nn.Conv2d(
            in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=0
        )

        # Calculate conv output size dynamically to avoid shape mismatches
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = F.relu(self.conv1(x))  # (N,32,H1,W1)
        x = F.relu(self.conv2(x))  # (N,64,H2,W2)
        x = F.relu(self.conv3(x))  # (N,64,H3,W3)

        # Flatten: (N, 64, H, W) → (N, 64*H*W)
        x = x.view(x.size(0), -1)
...
class PPOActorCritic(nn.Module):
    ...
    def __init__(self, num_actions: int, feature_dim: int = 512):
        """
        Args:
...
class PPOTrainer:
    ...
    # ---------- Helper to preprocess and convert state to tensor ----------

    def _preprocess_frame(self, frame: np.ndarray) -> np.ndarray:
        """
        Preprocess Atari frame following the paper's recommendations.
        """
    ...
    def _state_to_tensor(self, state: np.ndarray) -> torch.Tensor:
        """
        Preprocess and convert raw env state → (1,1,84,84) tensor on device.
        
        Args:
            state: (210, 160) or (210, 160, 1) grayscale frame
            
        Returns:
            (1, 1, 84, 84) tensor normalized to [0, 1]
        """
        # Handle both (H, W) and (H, W, 1) formats
        if state.ndim == 3:
            state = state.squeeze(-1)  # (210, 160, 1) → (210, 160)
        
        # Preprocess: crop and resize to 84x84
        state = self._preprocess_frame(state)  # (210, 160) → (84, 84)
        
        # Convert to tensor and normalize to [0, 1]
        state_tensor = torch.from_numpy(state).float() / 255.0
        
        # Add batch and channel dims: (84, 84) → (1, 1, 84, 84)
    ...
    def collect_rollout(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for step in range(self.rollout_steps):
            ...
            # Store in rollout buffer (preprocess first to 84x84)
            processed_state = self._preprocess_frame(self.state.copy() if isinstance(self.state, np.ndarray) else self.state)
            self.rollout_buffer.add(
                state=processed_state,  # Now storing preprocessed 84x84 state
                action=action_np,
                log_prob=log_prob,
                reward=reward,
                value=value,
    ...
    def update_policy(self, advantages: torch.Tensor, returns: torch.Tensor):
        ...
        """
        # Convert stored states → (T,1,84,84) 
        states_list = []
        for state in self.rollout_buffer.states:
            # States are already preprocessed to 84x84
            st = torch.from_numpy(state).float() / 255.0  # normalize
            st = st.unsqueeze(0).unsqueeze(0)  # (1,1,84,84)
        ...
        for epoch in range(self.epochs_per_update):
            ...
            for start_idx in range(0, batch_size_total, self.batch_size):
                ...
                batch_states = states[batch_indices]  # (B,1,84,84)
...
if __name__ == "__main__":
    ...
    # Preprocessing to 84x84 happens in PPOTrainer._preprocess_frame()
