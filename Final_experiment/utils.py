...
def load_all_results(results_dir: str) -> Dict[str, Dict]:
    ...
    for filename in sorted(os.listdir(logs_dir)):
        # Only process episode CSV files (not _updates.csv)
        if not filename.endswith(".csv") or "_updates" in filename:
            continue
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        # Default to "results" if it exists
        if os.path.isdir("results"):
            results_dir = "results"
        else:
            print("Usage: python utils.py <results_dir>")
            print("\nExample:")
            print("  python utils.py results")
            print("  python utils.py results_clip02_gae098")
            sys.exit(1)
